...
def CBR(plane, kernel_size, stride=1, padding=0):
    return nn.Sequential(
        xnn.Conv2d(plane, kernel_size, stride, padding, bias=False), xnn.BatchNorm2d(), nn.ReLU(inplace=True)
    )
...
class BasicBlock(xnn.Module):
    def __init__(self, plane):
        ...
        self.body = nn.Sequential(
            CBR(plane // 2, kernel_size=1, stride=1, padding=0), CBR(plane, kernel_size=3, stride=1, padding=1)
        )
