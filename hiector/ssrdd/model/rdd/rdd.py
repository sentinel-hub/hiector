...
from .utils.modules import DetPredict, FeaturePyramidNet
...
class RDD(xnn.Module):
    def __init__(self, backbone, cfg):
        ...
        cfg.setdefault("conf_thresh_2", 0.4)
        ...
        self.conf_thresh_2 = cfg["conf_thresh_2"]
    ...
    def restore(self, path):
        weight = torch.load(path) if torch.cuda.is_available() else torch.load(path, map_location=torch.device("cpu"))
    ...
    def forward(self, images, targets=None):
        ...
        if self.training:
            ...
            return detect(
                pred_cls, pred_loc, anchors, self.variance, self.conf_thresh, self.nms_thresh, top_n, self.conf_thresh_2
            )
