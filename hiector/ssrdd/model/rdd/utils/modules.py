...
class FeaturePyramidNet(xnn.Module):
    def __init__(self, depth, plane):
        ...
        for i in range(depth):
            self.link.append(nn.Sequential(xnn.Conv2d(plane, 1, 1, 0, bias=False), xnn.BatchNorm2d()))
            if i != depth:
                self.fuse.append(
                    nn.Sequential(nn.ReLU(inplace=True), xnn.Conv2d(plane, 3, 1, 1, bias=False), xnn.BatchNorm2d())
                )
